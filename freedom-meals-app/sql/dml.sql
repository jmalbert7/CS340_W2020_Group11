...
SELECT `Recipes`.`recipe_id`, `recipe_name`, `time`, `difficulty`, IFNULL(TRUNCATE(AVG(`rating`), 1), 'Not Yet Rated') AS `rating` FROM `Recipes` 
...
SELECT `customer_id`, `first_name`, `last_name`, `email`, `password`, `phone`, `admin` FROM `Customers`
...
SELECT `Recipes`.`recipe_id`, `Recipes`.`recipe_name`, `Recipe_Ratings`.`rating`, DATE_FORMAT(`Recipe_Ratings`.`date_rated`, '%m/%d/%Y') AS `date_rated` FROM `Recipes_in_Orders` 
...
-- Check a given email address to see if a user is already registered in the system with an account
SELECT `email` FROM `Customers` WHERE `email` = :emailInput;

-- Select the recipe name for a given recipeId, this is used to display items in the cart
SELECT `recipe_name` FROM `Recipes` WHERE `recipe_id` = :recipeIdInput;
...
(:fnameInput, :lnameInput, :emailInput, :passwordInput, :phoneInput, '1');

-- Add record to Recipes when admin adds a recipe
INSERT INTO `Recipes` (`recipe_name`, `time`, `difficulty`, `ingredients`) VALUES
(:recipenameInput, :recipetimeInput, :recipedifficultyInput, :recipeingredientsInput); 
